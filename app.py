...
<style>
    ...
    .rank-row {
        display: grid;
        grid-template-columns: 44px 64px 1fr 110px 130px;
        align-items: center;
        gap: 14px;
        padding: 12px 16px;
        border-radius: 6px;
...
with tab_model:
    ...
    demo_precision_at_10 = 0.78
    demo_recall_at_10 = 0.64
    demo_map = 0.71
    demo_coverage = 0.86
...
with tab_ratings:
    ...
    if filtered.empty:
        ...
        with st.spinner("Fetching posters..."):
            poster_lookup = {
                title: fetch_movie_details(title)
        ...
        for i, row in filtered.iterrows():
            ...
            st.markdown(f"""
                <div class="rank-row">
                    ...
                    <img class="rank-poster" src="{poster_url}" alt="{row['title']}">
...
with tab_about:
    ...
    st.markdown("""
        <div class="section-row">
            ...
            <p class="section-header">About CineMatch</p>
        </div>
